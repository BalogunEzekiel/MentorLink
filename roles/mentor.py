...
def show():
    ...
    with tabs[0]:
        ...
        st.write(f"- 🗕️ Total Sessions: **{len(total_sessions)}**")
                ...
                with st.expander(f"Request from {mentee_email}"):
                    ...
                    **Goals:** {mentee_profile.get("goals", "N/A")}
                    ...
                    if col1.button("✅ Accept", key=f"accept_{req_id}"):
                        available_slot = supabase.table("availability") \
                            .select("*").eq("mentorid", mentor_id).order("start", desc=False).limit(1).execute().data

                        if not available_slot:
                            st.warning("No availability to assign session.")
                        else:
                            ...
                            success, msg = create_session_with_meet_and_email(
                                supabase, mentor_id, mentee_id, start, end, availabilityid=availabilityid
                            )

                            if success:
                                supabase.table("mentorshiprequest").update({"status": "ACCEPTED"}) \
                                    .eq("mentorshiprequestid", req_id).execute()
                                supabase.table("availability").delete().eq("availabilityid", availabilityid).execute()
                                st.success("Request accepted and session scheduled.")
                                st.rerun()
                            else:
                                st.error(msg)
    ...
    with tabs[3]:
        ...
        sessions = supabase.table("session") \
            .select("*, availability(start, end), users!session_menteeid_fkey(email)") \
            .eq("mentorid", mentor_id).execute().data or []

        if sessions:
            for s in sessions:
                mentee_email = s.get("users", {}).get("email", "Unknown")
                start_str = s.get("availability", {}).get("start")
                end_str = s.get("availability", {}).get("end")
                ...
                if st.button("📧 Send Reminder", key=f"reminder_{s['sessionid']}"):
                    if send_email(
                        ...
                        subject="🗓️ Mentorship Session Reminder",
